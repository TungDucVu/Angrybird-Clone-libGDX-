...
public class LevelEndScreen implements Screen {
    ...
    public LevelEndScreen(Main game) {
        this.game = game;
        show();
    }
    ...
    public void render(float delta) {
        ...
        if (!isTransitioning) {
            ...
            game.spriteBatch.draw(levelClear, 265, 0, levelClear.getWidth()/2f, levelClear.getHeight()/2f);
            
            menuCircle.set(340 + 35, 72 + 35, 35);
            restartCircle.set(455 + 35, 72 + 35, 35);
            nextCircle.set(545 + 35, 70 + 35, 35);
          
            
            if (menuCircle.contains(touchPos.x, touchPos.y)) {
                game.spriteBatch.draw(menu, 340, 72, 70, 70); 
            } else {
            	game.spriteBatch.draw(menu1, 340, 72, 70, 70); 
            }
            
            if (restartCircle.contains(touchPos.x, touchPos.y)) {
                game.spriteBatch.draw(restart, 445, 72, 70, 70); 
            } else {
            	game.spriteBatch.draw(restart1, 445, 72, 70, 70); 
            }
            
            if (nextCircle.contains(touchPos.x, touchPos.y)) {
                game.spriteBatch.draw(next, 545, 70, 70, 70); 
            } else {
            	game.spriteBatch.draw(next1, 545, 70, 70, 70); 
            }
